...
donors <- xap.read_table("donors_clean")  %>% filter(icgc_donor_id %in% use_donors)
snp <- xap.read_table("snp_clean") %>% filter(icgc_donor_id %in% use_donors, chromosome != "MT")
struct <- xap.read_table("struct_clean")  %>% filter(icgc_donor_id %in% use_donors) 
cnv <- xap.read_table("cnv_clean") %>% filter(icgc_donor_id %in% use_donors)
...
server <- function(input, output, session) {
  ...
  re_values <- reactiveValues(scaling_factors = rep(1, 24), previous_radians = radians_f,
  ...
  snp_plot_data <- reactive({
    ...
    points <- fit_points(snp_data$chromosome, snp_data$chromosome_start, snp_data$transcripts,
                         0.8, 0.6, seq_df(),
                         metadata = snp_data[, c("transcripts", "mutation_type", "gene_affected", "chromosome",
...
#     }
  ...
  tooltip_fun <- function(data, session) {
    ...
    if("mutation_type" %in% names(data)) {
      ...
      paste0(
        ...
        "   Ensembl ID: ", paste0("<a href=",ensembl_gene_url,row$gene_affected," target=_blank>",row$gene_affected,"</a>"),
        ' <a href="',"javascript:pick_gene('",row$gene_affected,"','", row$mutation_type ,"','", row$mutated_from_allele,' > ',row$mutated_to_allele ,"','", row$chromosome ,"','", row$chromosome_start ,"','", row$chromosome_end ,"','", record_colour ,"')",'">Show</a>', "<br>",
  ...
  ggvis() %>% 
    ...
    layer_paths(data = cnv_line_data %>% group_by(seq), ~sin(theta) * r, ~cos(theta) * r, interpolate := "basis",
                strokeWidth := 0.8, opacity := ~opac) %>%
    layer_paths(data = cnv_plot_data %>% group_by(theta), ~sin(theta) * r, ~cos(theta) * r, stroke = ~copy_number,
    ...
    layer_points(data = snp_plot_data, ~sin(theta) * r, ~cos(theta) * r, fill = ~mutation_type, shape = ~is_clinvar,
    ...
    layer_paths(data = struct_plot_data, ~sin(theta) * r, ~cos(theta) * r, stroke = ~name_from,
                strokeWidth := ~opac, strokeWidth.hover := 2, strokeOpacity := 1, interpolate := "basis") %>%
    layer_text(data = text_df, ~sin(theta) * r, ~cos(theta) * r, text := ~seq, align := "center", baseline := "middle",
